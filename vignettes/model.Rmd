...
* $\begin{bmatrix} \xi \\ \tau \end{bmatrix}\sim\mathcal{N}\left(\begin{bmatrix}0 \\ 0 \end{bmatrix}, \begin{bmatrix} 1, \rho\sigma_\tau \\ \rho\sigma_\tau, \sigma^2_\tau \end{bmatrix}\right)$;
* $\log T_{ij}^E|\tau \sim \mathcal{N}\left(\gamma_j-\tau, \lambda_j^{-2}\right)$ for all $i=1,\dots,n$ and $j=1,\dots, J_i$.
...
\end{align*}
Then, we model the joint probability of the data by  recursively conditioning on the past, i.e.
\begin{align*}
...
\begin{align}
    \prod_{t=1}^\infty p(\mathbf{X}_i(t), \mathbf{S}_i(t)| \mathcal{H}_i(t), \xi, \tau) = p(\mathbf{T}_i, \mathbf{X}_i, \mathcal{P}_{i}(C_i)| \xi, \tau)
\end{align}
where $\mathcal{P}_{i}(C_i)=\{j\in\mathcal{P}_i\text{ such that  $r_{ij}=1$}\}$ and $r_{ij}=\mathbf{I}(\text{$T_{ij}^E < C_i$ and $X_{ij}\geq 18$})$. Then,
\begin{align}
    p(\mathbf{T}_i, \mathbf{X}_i, \mathcal{P}_{i}(t_i)| \xi, \tau) &= \prod_{j=1}^Jp(T_{ij}^E, X_{ij}, r_{ij}| \xi, \tau) = \prod_{j=1}^Jp(T_{ij}^E, X_{ij}| r_{ij}, \xi, \tau) p(r_{ijt_i}| \xi, \tau)\\
    &=\prod_{j=1}^J\left[\left\{p(T_{ij}^E=t, X_{ij}=x| \xi, \tau)\right\}^{r_{ij}}\left\{p(r_{ij}=0| \xi, \tau)\right\}^{1-r_{ij}}\right]\notag
\end{align}
where
\begin{align}
...
Let $L_i(t)=\mathbf{I}\big(\sum_{j\in\mathcal{P}_i}\sum_{u}^{t}S_{ij}(u) = J_i\big)$ indicates if the student $i$ has passed all the exams by day $t$. Then, it can be shown that the competing risk conditional submodel can be rewritten as
...
\end{align}
with $\lambda_{ik}(y|\xi, \tau)$ being the outcome-specific hazard function, with $k\in\mathcal{O}$. Namely
\begin{align}
    \lambda_{ik}(y|\xi, \tau)=
    \begin{cases}
